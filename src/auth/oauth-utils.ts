// OAuth helpers adapted from Cloudflare's remote-mcp-cf-access example.
...
export function generateCSRFProtection(request: Request): CSRFProtectionResult {
	const cookie = getCookiePolicy(request, "CSRF_TOKEN");
	const token = crypto.randomUUID();
	const setCookie = `${cookie.name}=${token}; ${cookie.attributes}; Max-Age=600`;
...
export function validateCSRFToken(formData: FormData, request: Request): ValidateCSRFResult {
	...
	const csrfCookie = cookies.find((c) => c.startsWith(`${cookie.name}=`));
	const tokenFromCookie = csrfCookie ? csrfCookie.substring(cookie.name.length + 1) : null;
	...
	const clearCookie = `${cookie.name}=; ${cookie.attributes}; Max-Age=0`;
...
export async function validateOAuthState(
	...
	try {
		const parsed: unknown = JSON.parse(storedDataJson);
		if (!isStoredOAuthState(parsed)) {
			throw new Error("Invalid stored OAuth state");
		}
		stored = parsed;
	} catch {
...
export async function addApprovedClient(
	...
	const cookie = getCookiePolicy(request, "APPROVED_CLIENTS");
	...
	return `${cookie.name}=${cookieValue}; ${cookie.attributes}; Max-Age=${String(THIRTY_DAYS_IN_SECONDS)}`;
}
...
async function getApprovedClientsFromCookie(
	...
	const cookies = cookieHeader.split(";").map((c) => c.trim());
	const targetCookie = cookies.find((c) => c.startsWith(`${cookie.name}=`));

	if (!targetCookie) return null;

	const cookieValue = targetCookie.substring(cookie.name.length + 1);
	...
	try {
		...
		if (!isStringArray(approvedClients)) {
			return null;
		}
...
function getCookiePolicy(
	request: Request,
	purpose: "APPROVED_CLIENTS" | "CSRF_TOKEN",
): { attributes: string; name: string } {
	const url = new URL(request.url);
	if (url.protocol === "http:" && isLoopbackHostname(url.hostname)) {
		return {
			attributes: "HttpOnly; Path=/; SameSite=Lax",
			name: `SKYDEO_DEV_${purpose}`,
		};
	}
...
async function verifySignature(
	...
	const enc = new TextEncoder();
	try {
		const bytePairs = signatureHex.match(/.{1,2}/g);
		if (bytePairs === null) return false;
		const signatureBytes = new Uint8Array(
			bytePairs.map((byte) => Number.parseInt(byte, 16)),
		);
...
export async function fetchUpstreamAuthToken(params: {
	...
	if (!response.ok) {
		return [
			null,
			null,
			new Response("Failed to exchange authorization code", {
	...
	if (!isTokenResponse(body)) {
		return [null, null, new Response("Invalid token response", { status: 502 })];
	}
...
function isTokenResponse(value: unknown): value is { access_token: string; id_token: string } {
	return (
		typeof value === "object" &&
		value !== null &&
...
function isStoredOAuthState(
	value: unknown,
): value is { oauthReqInfo: AuthRequest; codeVerifier: string } {
	return (
		isRecord(value) &&
		isAuthRequest(value.oauthReqInfo) &&
